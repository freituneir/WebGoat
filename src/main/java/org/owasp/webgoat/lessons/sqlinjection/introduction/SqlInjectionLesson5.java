...
public class SqlInjectionLesson5 implements AssignmentEndpoint {
  ...
  protected AttackResult injectableQuery(String query) {
    // Data control statements supplied by the client used to be executed as-is, which let anyone
    // change database privileges. Free-form client SQL is no longer executed; privileges are only
    // granted by the database administrator.
    return failed(this)
        .output("Statements supplied by the client are not executed, no privileges were changed.")
        .build();
  }
}
