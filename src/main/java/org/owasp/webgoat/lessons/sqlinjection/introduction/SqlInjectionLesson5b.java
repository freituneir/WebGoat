...
public class SqlInjectionLesson5b implements AssignmentEndpoint {
  ...
  protected AttackResult injectableQuery(String login_count, String accountName) {
    // Both values are numbers, so both are bound as numbers instead of being glued into the
    // statement: anything that is not a number never reaches the database.
    String queryString = "SELECT * From user_data WHERE Login_Count = ? and userid = ?";
    String executedQuery = displayQuery(login_count, accountName);
    try (Connection connection = dataSource.getConnection()) {
      ...
      try {
        ...
        if ((results != null) && (results.first() == true)) {
          ...
          if (results.getRow() >= 6) {
            return success(this)
                ...
                .output("Your query was: " + executedQuery)
                .feedbackArgs(output.toString())
                .build();
          } else {
            return failed(this)
                .output(output.toString() + "<br> Your query was: " + executedQuery)
                .build();
          }
        ...
        return failed(this)
            .output(sqle.getMessage() + "<br> Your query was: " + executedQuery)
            .build();
      }
    } catch (Exception e) {
      return failed(this)
  ...
  /** Renders the statement with the submitted values, for display in the lesson output only. */
  private static String displayQuery(String login_count, String accountName) {
    return "SELECT * From user_data WHERE Login_Count = "
        + login_count
        + " and userid = "
        + accountName;
  }
