...
public class JWTRefreshEndpointTest extends LessonTest {
  ...
  @Test
  void anotherUsersRefreshTokenCannotMintATokenForTom() throws Exception {
    ...
    // Presenting Tom's old access token together with Jerry's refresh token: the identity is taken
    // from the refresh token's own record, so the mismatch is refused.
